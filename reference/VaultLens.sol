...
contract VaultLens {
  function getAccountVaultInfo (WildcatMarket vault, address account) external view returns (AccountVaultInfo memory info) {

  }

  function getControlStatus (WildcatMarket vault) external view returns (ControlStatus memory status) {
  ...
  function getVaultsMetadata (address[] calldata vaults) external view returns (VaultData[] memory data) {
