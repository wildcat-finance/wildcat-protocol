...
contract StringPackerPrefixerTest is Test {
		...
		external
	{
		...
		vm.assume(prefixLength + suffixLength < 0x21);
		if (prefixLength == 0) {
	...
	function testGetStringOrBytes32AsString_Bytes32() external {
		(uint256 size, uint256 value) = lib.getStringOrBytes32AsString(
		...
		(size, value) = lib.getStringOrBytes32AsString(
	...
	function testPackUnpack() external {
		...
		assertEq(
			bytes(str),
			bytes(str2),
			'Unpacked string does not match original'
		);
	}
}
