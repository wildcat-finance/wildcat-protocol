...
contract MockSanctionsSentinel {
  ...
  function _deriveSalt(address account, address borrower, address token) internal pure returns (bytes32 salt) {
		return keccak256(abi.encode(account,borrower,token));
	}

  function getEscrowAddress(address account, address borrower, address token) external view returns (address) {
