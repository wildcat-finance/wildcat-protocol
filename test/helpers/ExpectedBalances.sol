...
contract NativeBalances {
  ...
  function sub(
    ...
    if (balance < amount) {
      revert(
        BalanceErrorMessages.insufficientNativeBalance(account, recipient, balance, amount, derived)
      );
    }
    return balance - amount;
  }

  function expectNativeTransfer(address from, address to, uint256 amount) public {
  ...
  function checkNativeBalances() internal view {
    ...
    for (uint256 j; j < accountsLength; j++) {
      ...
      if (expectedBalance != actualBalance) {
        revert(
          BalanceErrorMessages.nativeUnexpectedBalance(account, expectedBalance, actualBalance)
        );
      }
    }
  }

  function dumpNativeBalances() public view returns (NativeAccountDump[] memory accountBalances) {
    address[] memory accounts = accountsMap.keys();
    accountBalances = new NativeAccountDump[](accounts.length);
    for (uint256 i; i < accounts.length; i++) {
      address account = accounts[i];
      accountBalances[i] = NativeAccountDump(account, accountsMap.get(account));
    }
  }
}
...
contract ERC20Balances {
  ...
  function expectERC20Transfer(address token, address from, address to, uint256 amount) public {
    tokens.add(token);
    ...
    accounts.set(from, sub(token, from, to, fromBalance, amount, fromExists));
  ...
  function checkERC20Balances() internal view {
    ...
    for (uint256 i; i < length; i++) {
      ...
      EnumerableMap.AddressToUintMap storage accountsMap = tokenAccounts[token];
      address[] memory accounts = accountsMap.keys();
      uint256 accountsLength = accounts.length;
      for (uint256 j; j < accountsLength; j++) {
        address account = accounts[j];
  ...
  function dumpERC20Balances() public view returns (ERC20TokenDump[] memory tokenDumps) {
    uint256 length = tokens.length();
...
contract ERC721Balances {
  ...
  function expectERC721Transfer(
    ...
    (bool fromExists, uint256 fromBalance) = tokenData.accountBalances.tryGet(from);
    if (!fromExists) {
    ...
    (bool toExists, uint256 toBalance) = tokenData.accountBalances.tryGet(to);
    if (!toExists) {
  ...
  function checkERC721Balances() internal view {
    ...
    for (uint256 i; i < length; i++) {
      ...
      for (uint256 j; j < accountsLength; j++) {
        ...
        {
          uint256 expectedBalance = tokenData.accountBalances.get(account);
        ...
        uint256[] memory identifiers = tokenData.accountIdentifiers[account].values();
  ...
  function dumpERC721Balances() public view returns (ERC721TokenDump[] memory tokenDumps) {
  ...
  function dumpERC721Token(address token) internal view returns (ERC721TokenDump memory dump) {
    TokenData721 storage tokenData = tokenDatas[token];
    ...
    for (uint256 i; i < accountsLength; i++) {
      ...
      dump.accountIdentifiers[i] = tokenData.accountIdentifiers[account].values();
    }
  }
}
...
contract ERC1155Balances {
  ...
  function expectERC1155Transfer(ERC1155TransferDetails memory details) public {
    ...
    {
      EnumerableMap.UintToUintMap storage fromIdentifiers = tokenData.accountIdentifiers[
        details.from
      ];
      (bool fromExists, uint256 fromBalance) = fromIdentifiers.tryGet(details.identifier);
      if (!fromExists) {
        fromBalance = IERC1155(details.token).balanceOf(details.from, details.identifier);
      }
      fromIdentifiers.set(details.identifier, sub(details, fromBalance, fromExists));
    }

    {
      EnumerableMap.UintToUintMap storage toIdentifiers = tokenData.accountIdentifiers[details.to];
      (bool toExists, uint256 toBalance) = toIdentifiers.tryGet(details.identifier);
      if (!toExists) {
        toBalance = IERC1155(details.token).balanceOf(details.to, details.identifier);
      }
  ...
  function checkERC1155Balances() internal view {
    ...
    for (uint256 i; i < length; i++) {
      ...
      for (uint256 j; j < accountsLength; j++) {
        ...
        EnumerableMap.UintToUintMap storage accountIdentifiers = tokenData.accountIdentifiers[
          account
        ];
        ...
        for (uint256 k; k < identifiersLength; k++) {
          ...
          uint256 actualBalance = IERC1155(token).balanceOf(account, identifier);
          if (expectedBalance != actualBalance) {
            revert(
  ...
  function dumpERC1155Balances() public view returns (ERC1155TokenDump[] memory tokenDumps) {
    address[] memory tokensArray = tokens.values();
    uint256 length = tokensArray.length;
 ...
 */
contract ExpectedBalances is NativeBalances, ERC20Balances, ERC721Balances, ERC1155Balances {
