...
address constant LABELER_ADDRESS = address(uint160(uint256(keccak256('.labeler'))));

function setLabel(address account, string memory _label) {
  vm.store(LABELER_ADDRESS, bytes32(uint256(uint160(account))), LibString.packOne(_label));
}
...
{
  function(address) internal view returns (string memory) viewFn = getLabelView;
  assembly {
    pureFn := viewFn
  }
}
