  ...
  //////////////////////////////////////////////////////////////*/
	...
	function test_BalanceIncreasesOverTime() public {
		...
		uint256 interest = uint256(
			startBalance.rayMul(DefaultInterestPerSecondRay * SecondsIn365Days)
		);

		uint256 endBalance = wcDAI.balanceOf(wlUser);
		assertEq(endBalance, startBalance + interest);
		assertTrue(endBalance > startBalance, 'Balance did not increase');

		// The deposit of 1e18 is what ACTUALLY adjusts the scale factor,
		// the balanceOf call just projects it
		vm.prank(wlUser);
	...
	function test_BalanceIncreasesOverTimeWithFees() public {
		setupVault(1000);
		vm.prank(wlUser);
		wcDAI.deposit(50_000e18, wlUser);
    ...
    console2.log("Scaled supply: ", scaledTotalSupply);
		...
		uint256 interest = uint256(
			startBalance
				.rayMul(DefaultInterestPerSecondRay * SecondsIn365Days)
				.bipsMul(BipsOne - 1000)
		);
		uint256 fees = startBalance.rayMul(
			(DefaultInterestPerSecondRay * SecondsIn365Days).bipsMul(1000)
		);
		...
		vm.prank(wlUser);
		wcDAI.withdraw(2_499e18, wlUser);
		assertEq(wcDAI.accruedProtocolFees(), fees);
	...
	function test_WithdrawCollateral() public {
		...
		uint256 availableCollateral = wcDAI.maxCollateralToWithdraw();
		...
		uint256 availableCollateral2 = wcDAI.maxCollateralToWithdraw();
