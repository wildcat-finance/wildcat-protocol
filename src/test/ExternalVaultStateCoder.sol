...
contract ExternalVaultStateCoder {
		...
		)
	{
		(
			...
			scaleFactor,
			lastInterestAccruedTimestamp
		) = VaultStateCoder.getNewScaleInputs(_vaultState);
	}
	...
	function setNewScaleOutputs(
		...
		(_vaultState) = VaultStateCoder.setNewScaleOutputs(
			_vaultState,
			scaleFactor,
			lastInterestAccruedTimestamp
		);
	...
	{
		(scaledTotalSupply) = VaultStateCoder.getScaledTotalSupply(_vaultState);
	}

	function setScaledTotalSupply(uint256 scaledTotalSupply) external {
		(_vaultState) = VaultStateCoder.setScaledTotalSupply(
			_vaultState,
			scaledTotalSupply
		);
	}
	...
	function getScaleFactor() external view returns (uint256 scaleFactor) {
		(scaleFactor) = VaultStateCoder.getScaleFactor(_vaultState);
	}

	function setScaleFactor(uint256 scaleFactor) external {
		(_vaultState) = VaultStateCoder.setScaleFactor(_vaultState, scaleFactor);
	}
	...
	{
		(lastInterestAccruedTimestamp) = VaultStateCoder
			.getLastInterestAccruedTimestamp(_vaultState);
	}

	function setLastInterestAccruedTimestamp(uint256 lastInterestAccruedTimestamp)
		external
	{
		(_vaultState) = VaultStateCoder.setLastInterestAccruedTimestamp(
			_vaultState,
			lastInterestAccruedTimestamp
		);
	}
