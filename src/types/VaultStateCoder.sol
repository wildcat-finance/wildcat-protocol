...
VaultState constant DefaultVaultState = VaultState.wrap(0);
...
//////////////////////////////////////////////////////////////*/
		...
		)
	{
		assembly {
			annualInterestBips := signextend(
				0x01,
				shr(VaultState_annualInterestBips_bitsAfter, encoded)
			)
			scaledTotalSupply := and(
				MaxUint96,
				shr(VaultState_scaledTotalSupply_bitsAfter, encoded)
			)
			scaleFactor := and(
				MaxUint112,
				shr(VaultState_scaleFactor_bitsAfter, encoded)
			)
			lastInterestAccruedTimestamp := and(MaxUint32, encoded)
		}
	}
	...
	function encode(
		...
		assembly {
			if or(
				xor(annualInterestBips, signextend(1, annualInterestBips)),
				or(gt(scaledTotalSupply, MaxUint96), gt(scaleFactor, MaxUint112))
			) {
			...
			encoded := or(
				...
				or(
					shl(VaultState_scaledTotalSupply_bitsAfter, scaledTotalSupply),
					or(
						shl(VaultState_scaleFactor_bitsAfter, scaleFactor),
	...
	function setNewScaleOutputs(
		...
		assembly {
			if or(
				...
				gt(lastInterestAccruedTimestamp, MaxUint32)
			) {
				mstore(0, Panic_error_signature)
				mstore(Panic_error_offset, Panic_arithmetic)
				revert(0, Panic_error_length)
			...
			updated := or(
				and(old, VaultState_NewScaleOutputs_maskOut),
				or(
					shl(VaultState_scaleFactor_bitsAfter, scaleFactor),
					lastInterestAccruedTimestamp
				)
...
//////////////////////////////////////////////////////////////*/
	...
	function getAnnualInterestBips(VaultState encoded)
		internal
		pure
	...
	function setAnnualInterestBips(VaultState old, int256 annualInterestBips)
		internal
		pure
		returns (VaultState updated)
	{
		assembly {
			if xor(annualInterestBips, signextend(1, annualInterestBips)) {
				mstore(0, Panic_error_signature)
				mstore(Panic_error_offset, Panic_arithmetic)
				revert(0, Panic_error_length)
			}
			updated := or(
				and(old, VaultState_annualInterestBips_maskOut),
				shl(
					VaultState_annualInterestBips_bitsAfter,
					and(annualInterestBips, MaxInt16)
				)
...
//////////////////////////////////////////////////////////////*/
	...
	function getScaledTotalSupply(VaultState encoded)
		internal
		pure
	...
	function setScaledTotalSupply(VaultState old, uint256 scaledTotalSupply)
		internal
		pure
		returns (VaultState updated)
	{
		assembly {
			...
			updated := or(
				and(old, VaultState_scaledTotalSupply_maskOut),
				shl(VaultState_scaledTotalSupply_bitsAfter, scaledTotalSupply)
			)
		}
	}
...
//////////////////////////////////////////////////////////////*/
	...
	function setScaleFactor(VaultState old, uint256 scaleFactor)
		internal
		pure
		returns (VaultState updated)
	{
...
//////////////////////////////////////////////////////////////*/
	...
	function getLastInterestAccruedTimestamp(VaultState encoded)
		internal
		pure
	...
	function setLastInterestAccruedTimestamp(
		...
		assembly {
			updated := or(
				and(old, VaultState_lastInterestAccruedTimestamp_maskOut),
				lastInterestAccruedTimestamp
			)
		}
	}
...
//////////////////////////////////////////////////////////////*/
	...
	function isNull(VaultState a) internal pure returns (bool _isNull) {
