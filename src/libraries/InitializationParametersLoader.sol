...
function loadMetadataInitializationParameters() pure returns (address asset, bytes32 packedNamePrefix, bytes32 packedSymbolPrefix) {
    assembly {
        let freeMemPtr := mload(0x40)
        mstore(freeMemPtr, add(freeMemPtr, metadataReturnDataLength))
        returndatacopy(freeMemPtr, metadataReturnDataOffset, metadataReturnDataLength)
        asset := mload(freeMemPtr)
        packedNamePrefix := add(freeMemPtr, 0x20)
        packedSymbolPrefix := add(freeMemPtr, 0x40)
    }
}
function loadVaultConfigInitializationParameters() pure returns (
        address owner,
    ...
    assembly {
        ...
        returndatacopy(freeMemPtr, vaultConfigReturnDataOffset, vaultConfigReturnDataLength)
